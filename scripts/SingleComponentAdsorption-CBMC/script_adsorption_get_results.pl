  ...
  foreach (@temperature)
  {
    ...
    foreach (@molecule)
    {
      ...
      foreach $file (@files)
      {
        ...
        $insertion_percentage=`awk '/Performance of the swap addition move:/{getline; getline; printf substr(\$13,2)}' '$file'`;
        $deletion_percentage=`awk '/Performance of the swap deletion move/{getline; getline; printf substr(\$13,2)}' '$file'`;
        $reinsertion_percentage=`awk '/Performance of the Reinsertion move/{getline; getline; printf substr(\$13,2)}' '$file'`;

        printf DATw2 "$fugacity $fugacitycoefficient $press $absloading $absloading_molec_uc $absloading_mol_kg $absloading_mg_g $excloading $excloading_molec_uc $excloading_mol_kg $excloading_mg_g $heats $reinsertion_percentage $insertion_percentage $deletion_percentage\n";
      } 
      close(DATw2);
